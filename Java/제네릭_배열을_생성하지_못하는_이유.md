...
objectArray[0] = "타입이 달라 넣을 수 없다."; // ArrayStoreException (RuntimeException) 발생
```
...
```
## 배열은 실체화된다.
배열은 런타임에도 자신이 담기로 한 원소의 타입을 인지하고 확인한다. 그래서 Long 배열에 String을 넣으려 하면 ArrayStoreException이 발생한다. 반면, 앞서 이야기했듯 제네릭은 타입 정보가 런타임에는 소거된다. 
...
- 제네릭 타입(new List<E>[])  
- 매개변수화 타입, Parameterized Type(new List<String>[])  
- 타입 매개변수, Type Parameter(new E[])  
...
## 왜 제네릭 배열을 생성할 수 없을까 = 타입 안전하지 않기 때문이다.
